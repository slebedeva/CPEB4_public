...
snakemake --jobs 12 --cluster-config cluster_config.json --cluster "qsub -cwd -V -j yes -o {cluster.err} -m {cluster.m} -M {cluster.account} -pe smp {cluster.n} -l m_mem_free={cluster.h_vmem} -l h_rt={cluster.time} -N {cluster.name} -l data" --snakefile Snakefile --latency-wait 600 --rerun-incomplete --configfile config_parpipe.yaml
